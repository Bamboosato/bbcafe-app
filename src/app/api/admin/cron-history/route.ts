...
export async function GET(request: Request) {
  ...
  try {
    ...
    const [deleteRunsResult, sendRunsResult] = await Promise.allSettled([
      listCronRuns(limit),
    ...
    if (deleteRunsResult.status === "rejected" && sendRunsResult.status === "rejected") {
      return jsonError(503, "SERVICE_UNAVAILABLE", "Cron履歴を取得できません。", requestId);
    }

    return jsonData({ items: buildCronHistoryItems({ deleteRuns, limit, sendRuns }), warnings }, requestId);
  } catch (error) {
