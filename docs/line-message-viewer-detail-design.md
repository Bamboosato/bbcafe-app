...
- 保存期間を超えたメッセージは一覧/APIで非表示にする。
- Vercel Cronで期限切れメッセージを通常削除する。
...
- 期限切れ非表示は `expiresAt > now` で判定する。
- 自動削除前でも期限切れメッセージはAPIレスポンスに含めない。
...
Vercel Cronで日次実行し、期限切れメッセージを通常のFirestore deleteで削除する。
...
2. messages の総数を確認
3. 総数が1000件未満なら skipped として終了
4. expiresAt <= now のメッセージを sentAt asc で取得
5. 最大10000件まで削除
6. cronRuns に結果を保存
7. 削除件数、スキップ理由、失敗件数をログ出力
```

### 12.6 表示との関係

総数が1000件未満の場合、期限切れデータは物理削除されない。

ただし、閲覧者APIと管理者APIの通常一覧では以下で期限切れを除外する。

```text
expiresAt > now
```

そのため、物理削除前でも閲覧者には期限切れメッセージを表示しない。
...
| N-011 | 自動削除 | 総数1000件以上で期限切れメッセージを削除できる |
...
| B-003 | 自動削除 | 総数999件では削除しない |
| B-004 | 自動削除 | 総数1000件では削除判定を実行する |
| B-005 | 自動削除 | 期限切れ10000件まで削除する |
| B-006 | 自動削除 | 期限切れ10001件以上でも10000件までに制限する |
| B-007 | 一覧 | 同一 `sentAt` の投稿でも安定して表示できる |
| B-008 | 本文 | 長文テキストがUIを壊さない |
...
| 受信済み -> 期限切れ | `expiresAt <= now` で一覧から消える |
| 期限切れ -> 自動削除済み | Cronで物理削除される |
...
- 閲覧者は削除できない。
- 管理者はメッセージを削除できる。
- 期限切れメッセージは一覧/APIに表示されない。
- Vercel Cronで期限切れメッセージを通常削除できる。
- 総数999件では自動削除しない。
- 総数1000件以上では最大10000件/日まで削除する。
